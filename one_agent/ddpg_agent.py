...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, beta_start = 0.4, beta_episodes = 100):
    ...
    def act(self, state, eps = 0.0001, add_noise=True):
    ...
    def learn(self, experiences, gamma):
        ...
        actions_next = self.actor_target(next_states.unsqueeze(0))
        Q_targets_next = self.critic_target(next_states.unsqueeze(0), actions_next)
        ...
        loss = (((Q_targets - Q_expected).tanh().pow(2)).squeeze())*weights
        deltas = loss + eps
